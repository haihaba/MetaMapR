...
get.inchikey.RPAIRS<-function(type="main",url="https://raw.githubusercontent.com/slfan2013/metamapRsources/master/InchI%20Key%20Reaction%20Pairs"){ 
...
get.KEGG.pairs<-function(type="main",url="https://raw.githubusercontent.com/slfan2013/metamapRsources/master/KEGG%20PAIRS"){
  		# url="https://gist.github.com/dgrapov/4964564/raw/aec1a5097a3265d22109c9b34edd99a28f4012a3/KEGG+reaction+pairs"
		if(require(RCurl)==FALSE){install.packages("RCurl");library(RCurl)} else { library(RCurl)}
		text<-tryCatch( getURL(url,ssl.verifypeer=FALSE) ,error=function(e){NULL})
		tmp<-strsplit(text,"\\n")
		tmp2<-strsplit(as.character(unlist(tmp)), "\t")
...
get.GIST.csv<-function(url="https://raw.githubusercontent.com/slfan2013/metamapRsources/master/oxylipins%20edge%20list"){ 
...
get.CID.KEGG.pairs<-function(url="https://raw.githubusercontent.com/slfan2013/metamapRsources/master/CID_KEGG%20pairs"){
		if(require(RCurl)==FALSE){install.packages("RCurl");library(RCurl)} else { library(RCurl)}
		text<-tryCatch( getURL(url,ssl.verifypeer=FALSE) ,error=function(e){NULL})
		tmp<-strsplit(text,"\\n")
		tmp2<-strsplit(as.character(unlist(tmp)), "\t")
...
IDEOMgetR<-function(url="https://raw.githubusercontent.com/slfan2013/metamapRsources/master/IDEOM%20small%20list"){
		options(warn=-1)
		if(require(RCurl)==FALSE){install.packages("RCurl");library(RCurl)} else { library(RCurl)}
...
get.SDF.from.CID<-function(cids,DB=NULL,query.limit=25,update.DB=TRUE,save.as="CID.SDF.DB",
                           progress=TRUE,...){
